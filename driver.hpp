...
class PrototypeAST : public DefAST {
  private:
    ...
    bool valid_params;  // Flag per indicare se l'ordine dei parametri è valido
  
  public:
    PrototypeAST(std::string Name, std::vector<std::pair<std::string,ExprAST*>> Params);
    lexval getLexVal() const;
    void setext();
    const std::vector<std::pair<std::string,ExprAST*>>& getParams() const;
    void visit() override;
    int paramssize();
    bool isValid() const { return valid_params; }  // Nuovo metodo
