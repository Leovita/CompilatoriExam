...
/// PrototypeAST
PrototypeAST::PrototypeAST(std::string Name, std::vector<std::pair<std::string,ExprAST*>> Params) : 
Name(Name), External(false), Params(std::move(Params)) {
// Verifica ordine parametri
  valid_params = true;
  bool keyword_started = false;

  for(const auto& param : Params) {
    if(param.second != nullptr) { // param con default
      keyword_started = true;
    }
    else if(keyword_started) { // param senza default dopo uno con default
      valid_params = false;
      break;
    }
  }
...
const std::vector<std::pair<std::string,ExprAST*>>& PrototypeAST::getParams() const {
  return Params;
};
void PrototypeAST::visit() {
  if(!valid_params) {
    std::cerr << "Error: Invalid parameter order in function " << Name << std::endl;
    return;
  }
  ...
  *drv.outputTarget << drv.opening << Name << drv.closing << "[params ";
  ...
  for(const auto& param : Params) {
    *drv.outputTarget << drv.opening << param.first;
...
Function *PrototypeAST::codegen(driver& drv) {
  if(!valid_params) {
    LogErrorV("Invalid parameter order in function " + Name);
    return nullptr;
  }
  ...
  */
  ...
  FunctionType *FT = FunctionType::get(IntegerType::get(*context,32), intarray, false);
...
Function *FunctionAST::codegen(driver& drv) {
  ...
  if(!Proto->isValid()) {
    LogErrorV("Invalid parameter order in function definition");
    return nullptr;
  }
